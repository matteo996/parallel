...
void matrix_product(double **matrix1, double **matrix2, double **result_matrix, int N1, int N2, int M){
    ...
    int k,i,j; // rows and columns of result_matrix
    #pragma omp parallel for private (i,j,k)
    for(i=0; i<N1; i++){ //i: hold the row of m1 to multiply m1 [i][?]
        for(j=0; j<N2; j++){ //j: hold the column of m2 to be multiplied m2 [?][j]
            result_matrix[i][j] = 0;
                for(k=0; k<M; k++){
